  ...
  template: `
    <div class="auth-wrap">
      <div class="surface-card auth-card">
        <p class="section-eyebrow">Join the store</p>
        ...
        <label class="auth-field">
          ...
          <input type="text" class="form-control" autocomplete="name" [(ngModel)]="name" placeholder="Your name" />
        </label>

        <label class="auth-field">
          <span class="field-label">Email</span>
          <input type="email" class="form-control" autocomplete="email" [(ngModel)]="email" placeholder="you@example.com" />
        </label>
        ...
        <p class="auth-note">
          We send a confirmation link to your email. Google sign-in skips that step, since Google has
          already verified the address.
        </p>
        <p class="auth-alt">Already registered? <a routerLink="/login">Sign in</a></p>
      </div>
    </div>
  `,
  styles: [
    `
      ...
      .auth-note { font-size: 0.78rem; color: var(--text-subtle); margin: 0; text-align: center; line-height: 1.5; }
...
export class RegisterComponent {
  ...
  submit(): void {
    ...
    this.auth.register(this.name.trim(), this.email.trim(), this.password).subscribe({
      next: ({ user, notice }) => {
        this.busy.set(false);
        this.toast.success(notice ?? `Account created — welcome, ${user.name}`);
        this.router.navigate(['/shop']);
      },
      error: (e) => {
        this.busy.set(false);
        this.error.set(e?.error?.error ?? 'Could not create your account. Please try again.');
      },
    });
  }
}
